...
//
// Pattern: result = VMULbcx(m0, v) + VMULbcy(m1, v) + VMULbcz(m2, v) +
// VMULbcw(m3, v) Becomes: VMULAbcx ACC, m0, v
//          VMADDAbcy ACC, m1, v
//          VMADDAbcz ACC, m2, v
...
private:
  ...
  unsigned getBroadcastComponent(const MachineInstr &MI) const {
    switch (MI.getOpcode()) {
    case Mips::VMULbcx:
    case Mips::VMULbc_ISelx:
      return 0;
    case Mips::VMULbcy:
    case Mips::VMULbc_ISely:
      return 1;
    case Mips::VMULbcz:
    case Mips::VMULbc_ISelz:
      return 2;
    case Mips::VMULbcw:
    case Mips::VMULbc_ISelw:
      return 3;
    default:
      llvm_unreachable("Not a VMULbc instruction");
    }
  }
  ...
  unsigned getVMULAbcOpcode(unsigned Component) const {
    switch (Component) {
    case 0:
      return Mips::VMULAbcx;
    case 1:
      return Mips::VMULAbcy;
    case 2:
      return Mips::VMULAbcz;
    case 3:
      return Mips::VMULAbcw;
    default:
      llvm_unreachable("Invalid broadcast component");
    }
  }
  ...
  unsigned getVMADDAbcOpcode(unsigned Component) const {
    switch (Component) {
    case 0:
      return Mips::VMADDAbcx;
    case 1:
      return Mips::VMADDAbcy;
    case 2:
      return Mips::VMADDAbcz;
    case 3:
      return Mips::VMADDAbcw;
    default:
      llvm_unreachable("Invalid broadcast component");
    }
  }
  ...
  unsigned getVMADDbcOpcode(unsigned Component) const {
    switch (Component) {
    case 0:
      return Mips::VMADDbcx;
    case 1:
      return Mips::VMADDbcy;
    case 2:
      return Mips::VMADDbcz;
    case 3:
      return Mips::VMADDbcw;
    default:
      llvm_unreachable("Invalid broadcast component");
    }
  }
...
bool MipsR5900VU0AccChain::runOnMachineFunction(MachineFunction &MF) {
  ...
  LLVM_DEBUG(dbgs() << "Running R5900 VU0 ACC Chain pass on " << MF.getName()
                    << "\n");
...
bool MipsR5900VU0AccChain::buildChain(MachineInstr &Root,
  ...
  while (!Worklist.empty() && Depth < MaxChainDepth) {
    ...
    if (isVMULbc(*MI)) {
      // ACC multiply-add contraction is only legal when the source explicitly
      // permits contraction. The initial C kernel uses -ffast-math.
      if (!MI->getFlag(MachineInstr::MIFlag::FmContract))
        return false;
      ...
      // Replacing an arbitrary add tree with a linear ACC chain can reassociate
      // floating-point operations. Require both permissions at every node.
...
void MipsR5900VU0AccChain::transformChain(
    MachineBasicBlock &MBB, MachineInstr &Root,
    SmallVectorImpl<BroadcastMulLeaf> &Leaves,
    SmallVectorImpl<MachineInstr *> &VAdds) {
  LLVM_DEBUG(dbgs() << "R5900 VU0 ACC: Transforming chain with "
                    << Leaves.size() << " broadcast multiplies\n");
  ...
  BuildMI(MBB, InsertPt, DL,
          TII->get(getVMULAbcOpcode(Leaves[0].BroadcastComp)))
  ...
  for (size_t i = 1; i < Leaves.size() - 1; ++i) {
    BuildMI(MBB, InsertPt, DL,
            TII->get(getVMADDAbcOpcode(Leaves[i].BroadcastComp)))
        .addImm(DestMask)       // dest mask (xyzw)
  ...
  BuildMI(MBB, InsertPt, DL,
          TII->get(getVMADDbcOpcode(Leaves.back().BroadcastComp)), ResultReg)
      .addImm(DestMask)           // dest mask (xyzw)
